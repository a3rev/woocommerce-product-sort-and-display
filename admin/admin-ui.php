...
class Admin_UI
{
	...
	public function __construct() {
		$this->google_api_key_option     = WC_PSAD_KEY . '_google_api_key';
		$this->google_map_api_key_option = WC_PSAD_KEY . '_google_map_api_key';
		$this->toggle_box_open_option    = WC_PSAD_KEY . '_toggle_box_open';
		$this->version_transient         = WC_PSAD_KEY . '_licinfo';

		if ( defined( 'WC_PSAD_G_FONTS' ) ) {
			$this->is_load_google_fonts = (boolean) WC_PSAD_G_FONTS;
		}

		$this->support_url = 'https://a3rev.com/forums/forum/woocommerce-plugins/product-sort-and-display/';
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'woocommerce-product-sort-and-display' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'woocommerce-product-sort-and-display' ), $this->support_url );
			} else {
